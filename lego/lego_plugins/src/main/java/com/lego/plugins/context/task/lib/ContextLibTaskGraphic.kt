...
import com.lego.plugins.context.task.provider.getRedirectDestinationTask
...
class ContextLibTaskGraphic(
    ...
    fun graph(variant: BaseVariant, libTmpDirRoot: File) {
        whenGraph {
            /*
                ...
                (assembleProvider) <>>  assembleLibTask  <>>   publishLibTask
                                             ^                      ^
             */
            if(project.properties[Ext.LEGO_CLEAN_BEFORE_TASK] != false)
            ...
            if (!context.useResetCompiler) {
                ...
                assembleLibTask.graphicDependsOn((project.getKaptTask(variant.name) ?: throw LegoNoKaptTaskFoundException(
                    variant.name
                )).doLog())
            }
        ...
        project.legoConfigExtension.registerLibTask(
            assembleLibTask,
            publishLibTask,
            publishDevLibTask
        )
            ...
            */
            sourceTask?.let {
                ...
                sourceTask.graphicDependsOn(
                    project.getKaptTask(lib.variant.name) ?: throw LegoNoKaptTaskFoundException(lib.variant.name)
                )
            }
