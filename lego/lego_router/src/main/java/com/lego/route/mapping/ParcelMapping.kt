...
import com.lego.route.exception.ParcelableNoCreatorException
        ...
        if (null != creator) {
            Parcel.obtain().let { parcel ->
                ...
                return (creator.createFromParcel(parcel)).apply {
                    parcel.recycle()
                    Logger.da(" LEGO ") { " parcel rvalue : $this" }
                }
            }
        } else {
...
//                val result = RefArray.newInstance(genericType, size)
                ...
                Parcel.obtain().let { parcel ->
                    ...
                    return result.apply {
                        parcel.recycle()
                        Logger.da(" LEGO ") { " parcel array(${this.size}): $this" }
                    }
                }
            }
        }
    ...
    if (this is List<*> && type is ParameterizedType) {
        ...
        if (null != creator) {
            ...
            Parcel.obtain().let { parcel ->
                ...
                return result.apply {
                    parcel.recycle()
                    Logger.da(" LEGO ") { " parcel list(${this.size}): $this" }
                }
            }
        }
    }
